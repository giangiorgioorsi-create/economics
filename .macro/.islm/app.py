...
st.markdown("Ajuste los parámetros para observar el equilibrio. El botón de restablecer sincroniza sliders y gráficos.")

# --- BARRA LATERAL ---
with st.sidebar:
    st.header("Controles del Modelo")
    
    # BOTÓN DE RESET TOTAL: Limpia todo y reinicia la app
    ...
    # Definimos los valores iniciales exactos que acordamos
...
# Escalado dinámico para que el gráfico siempre sea visible
...
if 0 < i_star < limite_i:
    ...
    ax.annotate(f' Equilibrio\n Y*={Y_star:.0f}, i*={i_star:.2f}%', 
                ...
                arrowprops=dict(arrowstyle="->"), bbox=dict(boxstyle="round", fc="white", alpha=0.8))
...
col1.metric("Ingreso de Equilibrio (Y*)", f"{Y_star:.1f}")
col2.metric("Tasa de Interés (i*)", f"{i_star:.2f}%")

# Sistema de base para comparación (opcional, pero sin afectar el reset principal)
if 'base' not in st.session_state:
    st.session_state.base = {'Y': Y_star, 'i': i_star, 'G': G, 'M': ms}

if st.button("Fijar estos valores para comparar"):
    st.session_state.base = {'Y': Y_star, 'i': i_star, 'G': G, 'M': ms}

st.table({
    "Variable": ["Ingreso", "Interés", "Gasto", "Oferta Mon."],
